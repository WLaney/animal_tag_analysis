...
% Split pitch and roll between two processors
gcp;
res(1) = parfeval(@kalman_ag, 1, a_pitch, gyro(:,1), dt);
res(2) = parfeval(@kalman_ag, 1, a_roll, gyro(:,2), dt);
% Wait for results
for i=1:2
   % Block until one of these is done
   [ind, val] = fetchNext(res);
   if ind == 1
      comb_pitch = val ;
   else
      comb_roll = val;
